...
`_privacy.py` and `providers/_translation.py` are the ONLY modules that touch customer prompt content (the content-privileged allowlist). Never log, store, or concatenate prompt text outside these modules. CI-enforced by `tests/unit/test_privacy_firewall.py`.
