...
**Responses PR-C completion (2026-08-15):** The full Responses milestone is
complete: native OpenAI and Azure OpenAI create/stream/parse/helper coverage is
metered, and explicit `provider=` identity pins now bypass detection while
retaining SDK-shape and sync/async validation. The real-client E2E harness pins
native OpenAI at its local fake `/v1/responses` endpoint for sync and async
budget-check → dispatch → settlement coverage. These completion notes preserve
the older discovery/validation snapshots below as historical provenance.
...
| **Discovery: OpenAI Responses coverage is partial** | **PR-C complete 2026-08-15** | Native OpenAI and Azure OpenAI `responses.create(...)`, `responses.parse(...)`, and the new-response `responses.stream(...)` context-manager helper are now metered for sync and async clients through one budget-preflight and settlement lifecycle; `create(stream=True)` is included. Explicit provider identity pins bypass detection and make the native path E2E-testable behind a local/corporate gateway without weakening client-shape validation. Existing-response stream retrieval is a reviewed raw path with no duplicate charge. Beta/raw-response helpers, other Responses leaves, and non-Azure compatible profiles remain guarded `unmetered_spend`. The Agents SDK chat-completions pin is therefore optional when its path stays within the admitted native/Azure trio, while teams requiring strict coverage of every Responses path may still pin it. |
...
- **PR-C completion update 2026-08-15:** native OpenAI and Azure OpenAI `responses.create`, `responses.parse`, and the new-response `responses.stream` helper now use metered source-`BOTH` rules; `create(stream=True)` is included. Explicit provider identity pins and the real-client sync/async E2E harness complete the milestone without changing the surface contract. Missing or zeroed usage settles an explicitly marked request-length estimate rather than exact zero, with lease-backed calls retaining their reserved bound while output usage is unobservable. Existing-response stream retrieval remains raw with no duplicate charge. Beta/raw-response helpers, other leaves, and non-Azure compatible profiles remain guarded `unmetered_spend`, so the Agents SDK pin is optional for paths limited to the admitted trio and remains available for teams requiring strict full coverage.
