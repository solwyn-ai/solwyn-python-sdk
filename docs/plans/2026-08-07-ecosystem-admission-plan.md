...
| **Discovery: OpenAI Responses coverage is partial** | **Milestone update 2026-08-15** | Native OpenAI `responses.create(...)` is now metered for sync and async clients, including `stream=True`, through the budget-preflight and settlement lifecycle. `responses.parse` and the `responses.stream()` context-manager helper remain guarded `unmetered_spend` leaves until the coverage-completion tasks; Azure and other compatible profiles are not claimed. The Agents SDK chat-completions pin is therefore optional for native clients whose path only needs create/create-stream metering, while teams requiring strict coverage of every Responses path may still pin it. |
...
- **Milestone update 2026-08-15:** native OpenAI `responses.create` and `create(stream=True)` now use metered source-`BOTH` rules. Parse, the stream helper, beta/raw-response, other leaves, Azure, and compatible profiles remain guarded `unmetered_spend`, so the Agents SDK pin is optional for create-only paths and remains available for teams requiring strict full coverage.
...
**Interfaces:**
- Consumes: Task 2 admission; native OpenAI Responses create/create-stream metering plus the residual guarded `unmetered_spend` posture for parse/helper/other leaves (Task 3, superseded); Task 4 `start_run`.
...
# OPTIONAL at the 2026-08-15 milestone: native OpenAI responses.create and
# create(stream=True) are metered. Uncomment the conservative pin when your
# Agents SDK path may use responses.parse, responses.stream(), beta/raw-response,
# or other Responses leaves and you require strict full coverage:
...
```
  ...
  **Milestone guidance (2026-08-15):** `set_default_openai_api("chat_completions")` is optional for native OpenAI clients whose Agents SDK path only needs `responses.create` or `create(stream=True)`. It remains the conservative choice when strict coverage must include `responses.parse`, the `responses.stream()` helper, beta/raw-response access, or other leaves. No Azure/compatible Responses parity is claimed yet; the later Responses coverage-completion task updates this guidance when those paths actually ship.
...
| `with_options`/`copy`/residual Responses leaves bypass budgets under injection | **Milestone update 2026-08-15:** native OpenAI create/create-stream is metered, while parse, the stream helper, beta/raw-response, other leaves, Azure, and compatible profiles stay under the guarded unmetered posture. Teams needing strict full coverage may still pin Agents SDK to chat completions; Task 6 smoke will be the live escape detector and the rewrap contingency is pre-designed. |
...
- Responses API interception beyond native `create`/`create(stream=True)` — including parse, the stream helper, beta/raw-response, other leaves, Azure, and compatible profiles (coverage-completion work).
...
2. **`with_options`/`copy` and residual Responses leaves stay passthrough under the guarded posture in v1** instead of building interception/rewrap now. Native OpenAI `responses.create`/`create(stream=True)` is the metered exception as of 2026-08-15; parse, the stream helper, beta/raw-response, other leaves, Azure, and compatible profiles retain the warn/raise/allow posture. The planned smoke tests will make any real-world escape visible immediately; the rewrap design is written down and bounded.
