...
| **Discovery: the OpenAI Responses API surface is not metered** | **Still true, but no longer silent (updated 2026-08-13)** | Still no `responses` proxy on the wrapper, but the coverage strict-mode track (#54–#63) replaced the old warn-once map: `responses` is now a guarded namespace in the surface contract and every leaf (`responses.create`, `.parse`, `.stream`, …) is classified `unmetered_spend` — warn-once by default, `UntrackedSpendSurfaceError` in `on_unmetered="raise"` mode. Calls still bypass budget check and settlement, so the Agents SDK recipe MUST still pin `set_default_openai_api("chat_completions")`. Task 3's "make it loud" goal is already delivered — see its superseded status. |
...
6. **Escape hatches are already loud (landed since plan, 2026-08-13)** — `w.with_options(...)`, `w.copy()` (openai/anthropic), and `w.responses.*` still return/serve **unwrapped** paths, but the posture is no longer silent: the surface contract classifies `with_options`/`copy` as `unmetered_spend` with `capability_scope="client"` and `responses` as a guarded namespace with `unmetered_spend` leaves — warn-once by default, `UntrackedSpendSurfaceError` under `on_unmetered="raise"`. Task 3 is superseded (see its status). Under admission, injected clients hitting these from framework internals now warn or raise instead of escaping silently. The LangChain smoke test (Task 6) still asserts the enforced path is actually exercised per invoke, which catches any silent escape a framework takes. If a target framework routes model calls through `with_options`, escalate to the shared-core rewrap design sketched in Task 3 notes (deliberately not built in v1 — YAGNI until evidence).
7. **Streaming return type** — streaming returns `SyncStreamWrapper`/`AsyncStreamWrapper` (`stream.py`), not the provider's `Stream` class. Frameworks consume streams by iteration (duck-typed) — Agents SDK chat-completions streaming does. Documented limitation + covered by the Agents SDK streaming smoke test; making stream wrappers type-transparent is out of scope v1.
...
- **Version-churn testing without importing frameworks in core:** two-tier. (a) Unit tests exercise adapters against *protocol doubles* — tiny in-repo stand-ins mirroring the exact callback signatures — so `make test` never imports a framework (mirrors the sanctioned test-only-provider-SDK pattern, pyproject.toml:27,31 comments). (b) `tests/integration/frameworks/` smoke tests run against real latest frameworks in a scheduled + manually-triggered CI job (`frameworks-smoke.yml`), not in the PR gate. A smoke break means the framework moved — fix the thin adapter, never the contract.
...
**Validated 2026-08-13.** The surface contract now does what this task planned, with a stronger mechanism than the old warn-once map (`_UNSHIPPED_SPEND_SURFACES` and `_warn_unmetered_spend_surface_once` no longer exist):
...
- The plan's posture intent survives (warn, never refuse — by default), and the Responses API remains **unmetered**: Task 5's `set_default_openai_api("chat_completions")` pin is still REQUIRED.
...
**Interfaces:**
- Consumes: Task 2 admission, the responses `unmetered_spend` posture already shipped by the coverage track (Task 3, superseded), Task 4 `start_run`.
...
# REQUIRED: the Agents SDK defaults to the Responses API, which Solwyn does not
# yet meter (it warns once and passes through). Pin the chat-completions path
# so every model call is budget-checked and settled:
...
- [ ] **Step 2: Write the smoke test** — respx-stubbed OpenAI endpoint + stubbed control plane (reuse `conftest` doubles); asserts: (a) `set_default_openai_client` accepts the wrapper (no type rejection), (b) exactly one budget check per model call, (c) settled events carry `agent_run_id` with the expected `agent:` names and `parent_agent_run_id` = the workflow run. Marked `@pytest.mark.framework_smoke`, excluded from `make test`.
...
- [ ] **Step 5: Smoke test** (`test_langchain_smoke.py`, `framework_smoke` marker): real langchain + `ChatOpenAI` with the Solwyn-wrapped client injected (`root_client=`/`client=` fields — pydantic admission is the Task 2 win) + the handler; respx-stubbed endpoints; asserts one budget check per invoke (this is the with_options-escape detector from Task 3's contingency) and events scoped to the chain's run. Cover: sync chain, async chain, two-node LangGraph graph.
...
- [ ] **Step 4: Smoke test** — minimal one-agent crew against stubbed endpoints; attribution asserted. Enforcement caveat asserted honestly: without the custom-LLM recipe the model call is NOT budget-checked; the smoke pins that today's behavior is attribution-only so a future CrewAI client-injection feature flips the test loudly.
...
| `with_options`/`copy`/`responses` bypass budgets under injection | Landed (coverage track): surface-contract posture warns by default, strict-raises under `on_unmetered="raise"`; Task 6 smoke asserts one budget check per invoke (live escape detector); rewrap contingency pre-designed |
| Framework churn breaks adapters | Adapters are attribution-only over the §2 contract; protocol-double unit tests keep `make test` framework-free; scheduled smoke CI catches drift without blocking PRs; Agents SDK gets no shipped code at all |
...
| ContextVar scoping misfires in exotic execution (threaded callbacks, parallel branches) | `run_inline = True`; non-LIFO closes degrade to a structural warning, never an exception into user pipelines; limitation matrix in docs; smoke covers sync/async/LangGraph |
| Double-wrap or wrapping a wrapper masquerading as a provider client | `type()`-based guard (lie-immune) raises `ConfigurationError` (Task 2) |
...
- Full Responses API interception/settlement (warn-once + recipe pin only; fast-follow candidate).
...
1. **`copy`/`deepcopy` return the same shared wrapper** rather than raising. A spend-enforcement handle must not fork lease/reporter identity, and deep-copying frameworks (CrewAI) would otherwise be re-rejected at admission — the failure this project exists to fix. Alternative (fail loud) preserved in git if you disagree.
2. **`with_options`/`copy`/`responses` stay passthrough (warn-once) in v1** instead of building interception/rewrap now. Since the plan was written, the coverage strict-mode track shipped exactly this posture (plus an opt-in strict `raise` mode), so the judgment call is now settled fact. The smoke tests make any real-world escape visible immediately; the rewrap design is written down and bounded.
3. **`close()` now forwards to the wrapped client** (fail-soft) — drop-in semantics once frameworks own the client's lifecycle.
4. **Adapters are attribution-only** — they never read framework token usage and never emit spend; the wrapped client is the single reporting path (prevents double-count by construction).
...
**Superseded (1 task):** Task 3 in full — the coverage strict-mode track replaced the planned warn-once map with the surface contract: `responses` = guarded namespace with `unmetered_spend` leaves (warn fires at the leaf, not the namespace hop); `with_options`/`copy` = scoped client escapes with acknowledgment tokens; `on_unmetered` warn/raise/allow (`config.py:59`); strict error `UntrackedSpendSurfaceError` (`exceptions.py:252`). §0's Responses row, §1 Q2 item 6, the §4 risk row, and judgment call 2 were updated to match. The load-bearing consequence survives: the Responses API is still unmetered, so Task 5's `set_default_openai_api("chat_completions")` pin remains REQUIRED.
