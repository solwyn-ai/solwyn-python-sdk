...
Native OpenAI `responses.create` is metered through the OpenAI-dialect
wildcard-plus-override contract: the OpenAI-specific rule uses source `BOTH`,
while the wildcard keeps compatible profiles at `unmetered_spend`.
`responses.parse`, the `responses.stream()` helper, every other Responses leaf,
...
- **OpenAI**: Two response shapes — Chat Completions (`prompt_tokens`/`completion_tokens`) vs Responses API (`input_tokens`/`output_tokens`). Detect via `hasattr(usage, 'prompt_tokens')`. Native Responses dispatch is duck-typed through `prepare_responses_call`; streaming sets `stream=True` but never `stream_options`, and the terminal event supplies `event.response.usage` plus `event.response.service_tier`
...
- `responses` is a conditional cached property: a visible native OpenAI namespace returns a sync/async proxy whose `create` calls `_intercepted_call(_surface="responses")`. The path is primary-only and builds one filtered effective-default mapping for both preflight and dispatch: chat-only defaults are stripped and caller kwargs win. Effective `background=True` fails loud, including OpenAI `extra_body` precedence. Other providers and all non-`create` leaves resolve through the shared surface policy
