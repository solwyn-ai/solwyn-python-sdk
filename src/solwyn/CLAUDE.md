...
Native OpenAI `responses.create` and `responses.parse` are metered through the
OpenAI-dialect wildcard-plus-override contract: OpenAI-specific rules use
source `BOTH`, while the wildcard keeps compatible profiles at
`unmetered_spend`. The `responses.stream()` helper, every other Responses leaf,
Azure, and all other compatible profiles remain `unmetered_spend` at this
milestone.
...
- **OpenAI**: Two response shapes — Chat Completions (`prompt_tokens`/`completion_tokens`) vs Responses API (`input_tokens`/`output_tokens`). Detect via `hasattr(usage, 'prompt_tokens')`. Native Responses dispatch is duck-typed through `prepare_responses_call`: the leaf routes to `responses.create` or `responses.parse`; create streaming sets `stream=True` but never `stream_options`, parse refuses streaming before dispatch, and a create terminal event supplies `event.response.usage` plus `event.response.service_tier`
