...
- `budget.py` — `BudgetEnforcer` / `AsyncBudgetEnforcer` with cloud API check + outage posture. The SDK never prices a call: on the legacy per-call path an unreachable control plane either admits fail-open and TALLIES the call in the shared base (`_record_legacy_uncounted` — one call + its input estimate, trued up to the provider-reported total by `settle_uncounted` from the client's unkeyed settlement sites via a bounded per-call map; media without token usage keeps its estimate), or, under `fail_open=False`, fails closed (`deny_source="local_enforcement"`, `deny_reason="control_plane_unreachable"`); the sticky prior hard deny / retained run stop still wins first. The tally rides the next `/budgets/check` as `uncounted_calls`/`uncounted_tokens` (omitted at zero), at most one report on the wire at a time. Core folds BEFORE evaluating, so the report is KEPT only on 408/409/429 (408 and the 429 rate limiter answer before the route body, so before the fold; a 409 is a refusal that did not record the report), any 5xx, transport error/timeout, breaker hold, or cancellation (at-least-once); any 2xx (parsed or not) or other 4xx clears it (`_uncounted_verdict_for_check_error`; a 4xx drop warns, rate-limited). An all-zero settlement (abandoned stream) floors at the estimate. `_finish_uncounted_report` runs in a nested finally so a raising `release_probe` cannot strand the in-flight marker. The tally rides only `/budgets/check` — no exit flush. A lease cold start tallies on the lease ledger instead (`tally_uncounted=False`), never both. Settlement lives in the reporter, NOT here: `build_confirm_request` (sans-I/O, keyword-only) constructs the confirm; there is no `confirm_cost`. The `/budgets/check` POST rides the injected control-plane breaker. Run-scoped, token-billed calls meet the LEASE branch first (`_check_lease`): a sticky hard deny keeps the run on the per-call path, otherwise `_lease.admit()` decides and the enforcer performs only the I/O it prescribes — a blocking `POST /budgets/lease` grant on the caller's `budget_check_timeout` when there is no usable lease (one in-flight grant per run; losers take the legacy path rather than storm or wait), an ALWAYS-async renewal (daemon thread / task) at 75% depletion or the refresh deadline, and a best-effort surrender on `close()`, at interpreter exit, at RUN-SCOPE EXIT (`surrender_run`, driven by `_lifecycle._surrender_run`), and on every drop that ENDS the holder's use of a lease it can still name — an ineligible or denied renewal, an unreadable lease block, a run-stop directive (never a 404/409 renewal, which has nothing to release or a live successor that would refuse it, and never the client-side expiry, whose next admission simply re-grants). A drop release is parked under `_state_lock` (`_owe_release_locked`) and posted OFF the caller's thread by the concrete enforcer (daemon thread / task); a release that cannot be dispatched stays in a bounded queue for retry/close. Each enforcer has four release workers and 64 pending items, reuses a dedicated pool (async origin TLS initialization runs in one daemon thread), coalesces identical payloads, and counts queue saturation/unsent expiry in `release_counts`. `_await_run_release` retains every active identity until actual request completion; an exhausted wait routes to per-call admission rather than overtaking a still-active release. Close uses the same bounded dispatcher and retains pool ownership until outstanding work ends; async cancellation callbacks never resurrect workers during loop shutdown. `_SURRENDER_TIMEOUT_S` is 3s with ONE retry on a timeout only (the route is idempotent; a refusal is an answer). Every lease call rides `_control_plane_breaker` exactly like the check, and a REFUSAL (409 cap, 503 lease_unavailable, malformed body) credits the breaker and routes the run to the legacy path — only transport failures are an outage. A run-stop directive echoed for the WRONG run is contract drift on every channel (check, grant, renew): the breaker is credited, a distinct ERROR fires (`budget.check_directive_misrouted` / `lease.grant_directive_misrouted` / `lease.renew_directive_misrouted`), and only that one call degrades down the outage posture ladder. An authoritative grant/renew denial feeds the UNCHANGED sticky-deny machinery; `BudgetCheckResult.lease_id` is the settlement key that later trues the reservation up — except for a call whose usage was never measurable (`build_confirm_request(usage_unmeasured=True)`, the client's fail-soft synthetic tier), which settles AT its reserved bound so a paid response's spent output allowance is never re-lent past the run's cap. The enforcer also owns the two things the sans-I/O ledger cannot: the §8 uncounted-mode telemetry (`_note_uncounted_admission` — one WARNING on ENTRY to a fail-open uncounted episode, then at most 1/30s while it persists; an installed grant ends the episode, both the cold-start and the expiry-ladder admissions feed it) and the fork identity — `_reset_after_fork_in_child` mints a FRESH `holder_id`, because the client's `_solwyn_sdk_instance_id` survives a fork and the server releases a same-(project, run, holder) active lease as stale, so a child re-granting under the parent's id would kill the parent's live lease and start a churn
- `_lease.py` — sans-I/O lease ledger (PJ-2): the §4 outage ladder, atomic reservation math, generation fencing, renewal/backoff bookkeeping, the sticky `spend_unknown` pin (`mark_spend_unknown`: a failed-over post-send-ambiguous hop keeps the call's live reservation, but no later true-up, release, or sweep may refund below its bound), and the RELEASE owed for a terminal drop (`apply_grant_response` returns it alongside the outcome; `surrender_if_current` is the terminal twin of `drop_if_current` — which drops are worth releasing is decided at each call site, never inside `_drop_lease`). Takes NO locks and does NO I/O — the caller serializes it (the sync enforcer's `_state_lock`, the async enforcer's event loop) and supplies a monotonic `now`
...
- `_run.py` — `ContextVar`-backed agent-run scope; `run(name, tags=...)` public entry point; `current_run()` preserves the public `(id, name)` tuple while a private copied snapshot carries bounded tags through deferred reporting. Scope exit is a lease boundary: `_RunScope._exit` (when the frame popped is the scope's own, exception or not) and `RunHandle.finish()` call `_lifecycle._surrender_run(run_id)` — NOT each `activate()` exit, since a detached identity is re-entered by design
...
- ALL reservation settlement — streaming `on_complete` AND non-streaming chat/media success — fire-and-forgets via `reporter.report_settlement(confirm, event)` (confirm built sans-I/O by `budget.build_confirm_request`, enqueued with its metadata event as one ordered item). The event carries the same `lease_id` as its confirm, read from the admission result (`budget.lease_id`), on success AND error events of a lease-admitted call — never from the ledger's live state, so neither an asynchronous renewal nor a fresh grant taken by a later call on the same run can make the two halves disagree. Never blocks the user thread; there is no blocking `confirm_cost`
